...
function NoteCard({ note, type, subMode, onDelete, onRename }: { note: NoteRow; type: string; subMode: string; onDelete: (noteType: string, title: string) => void; onRename: (noteType: string, title: string, displayTitle: string) => void }) {
  ...
  return (
    <div className="rounded-xl border border-gray-200 bg-white p-4 hover:shadow-sm transition-shadow shrink-0 w-64 flex flex-col">
        ...
        <div className="flex items-start justify-between gap-1">
          {editing ? (
            ...
            <div className="flex items-center gap-1 min-w-0">
              <h3 className="font-semibold text-gray-900 text-sm leading-snug truncate">{note.displayTitle ?? note.title}</h3>
              <button onClick={startEditing} className="shrink-0 text-gray-300 hover:text-indigo-400 transition-colors" aria-label="Rename note">
                <Pencil size={11} />
              </button>
            </div>
          )}
          <button onClick={() => setConfirming(true)} className="shrink-0 text-gray-300 hover:text-red-400 transition-colors mt-0.5" aria-label="Delete note">
            <Trash2 size={13} />
          </button>
        </div>
